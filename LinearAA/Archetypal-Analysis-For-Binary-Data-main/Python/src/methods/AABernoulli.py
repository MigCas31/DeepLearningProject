...
def SUpdate(X,PC,S,SSt,n_arc,n_samples,device):
    ...
    for j in range(grid.shape[0]):
        ...
        # Clamp PC@S to avoid division by zero
        PCS = PC@S
        PCS = torch.clamp(PCS, min=1e-12, max=1.0-1e-12)
        
        K =  -(PC[:,grid[j]]).T@(X/PCS) + (PC[:,grid[j]]).T@((1-X)/(1-PCS))

        QuadS = torch.einsum('ji,jk,jl->ikl',PC[:,grid[j]],PC[:,grid[j]],X/(PCS**2)+(1-X)/((1-PCS)**2))
...
def CUpdate(X,P,C,S,PC,n_arc,PPP,device):
    ...
    for i in range(n_arc):
        ...
        # Clamp P@C@S to avoid division by zero
        PCS = P@C@S
        PCS = torch.clamp(PCS, min=1e-12, max=1.0-1e-12)

        v = ((X/(PCS**2))@S[i,:]**2) + ((((1-X)/((1-PCS)**2))@S[i,:]**2))

        grad = (P.T@((X/PCS)@S[i,:]) -P.T@(((1-X)/(1-PCS))@S[i,:]))

        xtilde  = (P*torch.sqrt(v[:,None]))
        Xty =  grad + xtilde.T@(xtilde@C[:,i])

        tol = 1e-9
        C_k,_,PPP[i] = fastnnls(xtilde,Xty,tol,C[:,i],PPP[i],device=device)
...
def LossFunc(X,PC,S,device):
    ...
    PCS = PC@S
    PCS = torch.clamp(PCS, min=1e-12, max=1.0-1e-12)
    loss = torch.multiply(-X,torch.log(PCS)).sum().sum() - torch.multiply((1-X),torch.log(1-PCS)).sum().sum() 
